...
<form method="post" action="options.php" class="AGB-settings">
  ...
  <?php do_settings_sections( Consts::SETTINGS_GROUP ); ?>
	...
	<main class="AGB-settings__content" id="AGB-blocks">
		<aside id="search" class="AGB-search">
		...
		<div class="AGB-list" id="AGB-blocks--custom">
			<dl class="list">
				<?php foreach( $categories as $key => $cat ): ?>
					<dt class="AGB-list__title">
						<?php echo $cat; ?>
						<?php
					...
					?>
					<dd class="AGB-block<?php if( $active ): ?> is-active<?php endif; ?>" id="<?php echo sanitize_title($block['id']); ?>">
						<header class="AGB-block__head">
							...
							<div class="AGB-block__title js-AGB-show-panel">
								<span class="AGB-block__name"><?php echo $block['name']; ?></span>
		...
		<div class="AGB-list" id="AGB-blocks--native">
			<dl class="list">
			<?php
				foreach($native_blocks as $cat => $blocks):
			?>
				<dt class="AGB-list__title"><?php echo $cat; ?></dt>
				<?php foreach($blocks as $block):
					$active = !in_array( $block['id'], $disabled_blocks );
				?>
					<dd class="AGB-block<?php if ( $active ) : ?> is-active<?php endif; ?>">
						<header class="AGB-block__head">
